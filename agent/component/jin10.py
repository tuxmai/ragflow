    ...
    def _run(self, history, **kwargs):
        ...
        try:
            if self._param.type == "flash":
                ...
                response = requests.get(url="https://open-data-api.jin10.com/data-api/flash?category=" + self._param.flash_type, headers=headers, data=json.dumps(params))
            ...
            if self._param.type == "calendar":
                ...
                response = requests.get(
                    url="https://open-data-api.jin10.com/data-api/calendar/" + self._param.calendar_datatype + "?category=" + self._param.calendar_type, headers=headers, data=json.dumps(params)
                )
            ...
            if self._param.type == "symbols":
                ...
                if self._param.symbols_datatype == "quotes":
                    params["codes"] = "BTCUSD"
                response = requests.get(url="https://open-data-api.jin10.com/data-api/" + self._param.symbols_datatype + "?type=" + self._param.symbols_type, headers=headers, data=json.dumps(params))
                response = response.json()
            ...
            if self._param.type == "news":
                ...
                response = requests.get(url="https://open-data-api.jin10.com/data-api/news", headers=headers, data=json.dumps(params))
                response = response.json()
                jin10_res.append({"content": pd.DataFrame(response["data"]).to_markdown()})
