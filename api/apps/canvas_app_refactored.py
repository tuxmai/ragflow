...
from api.utils.api_utils import (
    get_data_error_result, get_json_result, server_error_response, validate_request
)
...
class CanvasManager:
    ...
    def get_user_canvas_list(user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            canvas_list = [
                canvas.to_dict() 
                for canvas in UserCanvasService.query(user_id=user_id)
            ]
            
            # Sort by update time (newest first)
            sorted_canvas_list = sorted(
                canvas_list, 
                key=lambda canvas: canvas["update_time"], 
                reverse=True
            )
    ...
    def delete_canvases(user_id: str, canvas_ids: List[str]) -> Tuple[bool, Optional[str]]:
        ...
        try:
            for canvas_id in canvas_ids:
                # Validate ownership
                is_owner, error_msg = CanvasAuthorizationManager.validate_canvas_ownership(
                    user_id, canvas_id
                )
                if not is_owner:
                    return False, error_msg
        ...
        try:
            ...
            if "message" in execution_params:
                canvas.messages.append({
                    "role": "user", 
                    "content": execution_params["message"], 
                    "id": message_id
                })
            ...
            if is_streaming:
                return CanvasExecutionManager._execute_streaming(
                    canvas, canvas_service, canvas_id, message_id, running_hint_text
                )
            else:
                return CanvasExecutionManager._execute_non_streaming(
                    canvas, canvas_service, canvas_id, message_id, running_hint_text
                )
        ...
        def generate_streaming_response():
            ...
            try:
                for answer_chunk in canvas.run(running_hint_text=running_hint_text, stream=True):
                    ...
                    response_data = {
                        "answer": answer_chunk["content"], 
                        "reference": answer_chunk.get("reference", [])
                    }
                ...
                # Update canvas state
                CanvasExecutionManager._update_canvas_after_execution(
                    canvas, canvas_service, canvas_id, message_id, final_answer
                )
    ...
    def _execute_non_streaming(
        ...
        for answer_chunk in canvas.run(running_hint_text=running_hint_text, stream=False):
            ...
            final_answer["content"] = (
                "\n".join(answer_chunk["content"]) 
                if "content" in answer_chunk 
                else ""
            )
            ...
            return True, {
                ...
                "reference": final_answer.get("reference", [])
            }
        
        return False, {"message": "Canvas execution failed"}
    
    @staticmethod
    def _update_canvas_after_execution(
        canvas: Canvas, 
        canvas_service: Any, 
        canvas_id: str, 
        message_id: str, 
...
class CanvasDebugManager:
    ...
    def debug_component(
        ...
        canvas_id: str, 
        component_id: str, 
        debug_params: List[Dict[str, Any]]
    ) -> Tuple[bool, Any]:
        """
...
class CanvasVersionManager:
    ...
    def get_canvas_versions(canvas_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            versions = [
                version.to_dict() 
                for version in UserCanvasVersionService.list_by_canvas_id(canvas_id)
            ]
            
            # Sort by update time (newest first)
            sorted_versions = sorted(
                versions, 
                key=lambda version: version["update_time"], 
                reverse=True
            )
...
class TeamCanvasManager:
    ...
    def get_team_canvases(
        ...
        try:
            ...
            # Get canvases
            canvases, total = UserCanvasService.get_by_tenant_ids(
                tenant_ids, user_id, page_number, items_per_page, 
                order_by, is_descending, keywords
            )
...
def get_canvas_sse_endpoint(canvas_id: str):
    ...
    token = auth_parts[1]
    is_valid, tenant_id, error_msg = CanvasAuthorizationManager.validate_api_token_access(
        canvas_id, token
    )
...
def execute_canvas_endpoint():
    ...
    success, result = CanvasExecutionManager.execute_canvas(
        current_user.id, canvas_id, execution_params
    )
...
def get_input_elements_endpoint():
    ...
    success, result = CanvasDebugManager.get_input_elements(
        current_user.id, canvas_id, component_id
    )
...
def debug_canvas_component_endpoint():
    ...
    success, result = CanvasDebugManager.debug_component(
        current_user.id, canvas_id, component_id, debug_params
    )
...
def test_database_connection_endpoint():
    ...
    success, result_message = DatabaseConnectionManager.test_database_connection(
        connection_params
    )
...
def update_canvas_settings_endpoint():
    ...
    success, result = CanvasSettingsManager.update_canvas_settings(
        current_user.id, canvas_id, settings_data
    )
