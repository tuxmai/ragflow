...
def add_llm():
    ...
    if llm["model_type"] == LLMType.EMBEDDING.value:
        ...
        assert factory in ChatModel, f"Chat model from {factory} is not supported yet."
        mdl = ChatModel[factory](key=llm["api_key"], model_name=mdl_nm, base_url=llm["api_base"])
        try:
...
def my_llms():
    try:
        ...
        for o in TenantLLMService.get_my_llms(current_user.id):
            ...
            res[o["llm_factory"]]["llm"].append({"type": o["model_type"], "name": o["llm_name"], "used_token": o["used_tokens"]})
