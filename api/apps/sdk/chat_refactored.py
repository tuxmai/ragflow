...
from api.utils.api_utils import (
    check_duplicate_ids, get_error_data_result, get_result, token_required
)
...
class ChatAssistantManager:
    ...
    def create_chat_assistant(tenant_id: str, assistant_data: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if llm_config:
                is_valid, error_msg, processed_config = LLMConfigurationManager.process_llm_configuration(
                    llm_config, tenant_id
                )
                if not is_valid:
                    return False, {"message": error_msg}
            ...
            if assistant_data.get("rerank_id"):
                is_valid, error_msg = LLMConfigurationManager.validate_rerank_model(
                    assistant_data["rerank_id"], tenant_id
                )
                if not is_valid:
                    return False, {"message": error_msg}
            ...
            # Format response
            response_data = ChatAssistantManager._format_assistant_response(
                created_assistant.to_json(), assistant_data["dataset_ids"]
            )
    ...
    def update_chat_assistant(tenant_id: str, chat_id: str, update_data: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if prompt_config:
                ...
                # Validate prompt parameters
                is_valid, error_msg = PromptConfigurationManager.validate_prompt_parameters(
                    update_data["prompt_config"]
                )
                if not is_valid:
                    return False, error_msg
            ...
            if update_data.get("rerank_id"):
                is_valid, error_msg = LLMConfigurationManager.validate_rerank_model(
                    update_data["rerank_id"], tenant_id
                )
                if not is_valid:
                    return False, error_msg
            ...
            if "name" in update_data:
                ...
                if (update_data["name"].lower() != existing_data["name"].lower() and 
                    len(DialogService.query(
...
class ChatAssistantListManager:
    ...
    def list_chat_assistants(tenant_id: str, query_params: Dict[str, Any]) -> Tuple[bool, List[Dict[str, Any]]]:
        ...
        try:
            ...
            # Get assistants
            assistants = DialogService.get_list(
                tenant_id, page_number, items_per_page, order_by,
                is_descending, assistant_id, assistant_name
            )
...
def delete_chat_assistants_endpoint(tenant_id: str) -> Any:
    ...
    successful_deletions, errors, duplicate_messages = ChatAssistantDeletionManager.delete_chat_assistants(
        tenant_id, deletion_request
    )
    
    if errors:
        if successful_deletions > 0:
            return get_result(
                data={"success_count": successful_deletions, "errors": errors},
                message=f"Partially deleted {successful_deletions} chats with {len(errors)} errors"
            )
        else:
    ...
    if duplicate_messages:
        if successful_deletions > 0:
            return get_result(
                message=f"Partially deleted {successful_deletions} chats with {len(duplicate_messages)} errors",
                data={"success_count": successful_deletions, "errors": duplicate_messages}
            )
        else:
