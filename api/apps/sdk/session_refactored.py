...
from api.utils.api_utils import (
    get_result, token_required, get_data_openai, get_error_data_result, 
    validate_request, check_duplicate_ids
)
...
class ChatSessionManager:
    ...
    def create_chat_session(tenant_id: str, chat_id: str, session_data: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            dialog_query_result = DialogService.query(
                tenant_id=tenant_id, 
                id=chat_id, 
    ...
    def create_agent_session(tenant_id: str, agent_id: str, request_data: Dict[str, Any], files: Dict[str, Any], user_id: str) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            if preset_parameters:
                success, error_message = AgentSessionManager._process_preset_parameters(
                    preset_parameters, request_data, files, user_id
                )
                if not success:
...
class ChatCompletionHandler:
    ...
    def _create_streaming_response(tenant_id: str, chat_id: str, completion_request: Dict[str, Any]) -> Response:
        ...
        response = Response(
            rag_completion(tenant_id, chat_id, **completion_request), 
...
class OpenAICompatibilityHandler:
    ...
    def handle_openai_chat_completion(tenant_id: str, chat_id: str, openai_request: Dict[str, Any]) -> Response:
        ...
        if is_streaming:
            return OpenAICompatibilityHandler._create_openai_streaming_response(
                chat_id, dialog, processed_messages, openai_request
            )
        else:
            return OpenAICompatibilityHandler._create_openai_non_streaming_response(
                chat_id, dialog, processed_messages, openai_request
            )
    ...
    def _create_openai_streaming_response(chat_id: str, dialog: Any, messages: List[Dict[str, Any]], request_data: Dict[str, Any]) -> Response:
        ...
        def generate_streaming_response() -> Generator[str, None, None]:
            ...
            base_response["usage"] = {
                ...
                "total_tokens": len(prompt_text) + token_count
            }
...
class SessionListManager:
    ...
    def list_chat_sessions(tenant_id: str, chat_id: str, query_params: Dict[str, Any]) -> Response:
        ...
        conversations = ConversationService.get_list(
            chat_id, page_number, items_per_page, order_by, 
            is_descending, session_id, session_name, user_id
        )
...
class SessionDeletionManager:
    ...
    def delete_chat_sessions(tenant_id: str, chat_id: str, deletion_request: Optional[Dict[str, Any]]) -> Response:
        ...
        # Build response
        return SessionDeletionManager._build_deletion_response(
            successful_deletions, deletion_errors, duplicate_messages
        )
    ...
    def _build_deletion_response(successful_count: int, errors: List[str], duplicate_messages: List[str]) -> Response:
        ...
        if errors:
            if successful_count > 0:
                return get_result(
                    data={"success_count": successful_count, "errors": errors},
                    message=f"Partially deleted {successful_count} sessions with {len(errors)} errors"
                )
            else:
        ...
        if duplicate_messages:
            if successful_count > 0:
                return get_result(
                    message=f"Partially deleted {successful_count} sessions with {len(duplicate_messages)} errors",
                    data={"success_count": successful_count, "errors": duplicate_messages}
                )
            else:
...
def create_agent_session_endpoint(tenant_id: str, agent_id: str) -> Response:
    ...
    success, result = AgentSessionManager.create_agent_session(
        tenant_id, agent_id, request_data, files, user_id
    )
...
def update_chat_session_endpoint(tenant_id: str, chat_id: str, session_id: str) -> Response:
    ...
    success, error_message = ChatSessionManager.update_chat_session(
        tenant_id, chat_id, session_id, request_data
    )
...
def agents_openai_completion_endpoint(tenant_id: str, agent_id: str) -> Response:
    ...
    if request_data.get("stream", True):
        return Response(
            completionOpenAI(tenant_id, agent_id, question, session_id=session_id, stream=True),
            mimetype="text/event-stream"
        )
    else:
...
def agent_completions_endpoint(tenant_id: str, agent_id: str) -> Response:
    ...
    if request_data.get("stream", True):
        response = Response(
            agent_completion(tenant_id, agent_id, **request_data),
            mimetype="text/event-stream"
        )
        response.headers.add_header("Cache-control", "no-cache")
        response.headers.add_header("Connection", "keep-alive")
...
def list_agent_sessions_endpoint(tenant_id: str, agent_id: str) -> Response:
    ...
    # Get conversations
    conversations = API4ConversationService.get_list(
        agent_id, tenant_id, page_number, items_per_page, order_by,
        is_descending, session_id, user_id, include_dsl
    )
            ...
            yield "data:" + json.dumps(
                {
                    "code": 500,
                    "message": str(error),
                    "data": {"answer": f"**ERROR**: {str(error)}", "reference": []}
                },
                ensure_ascii=False
            ) + "\n\n"
...
def chatbot_completions_endpoint(dialog_id: str) -> Response:
    ...
    if request_data.get("stream", True):
        response = Response(
            iframe_completion(dialog_id, **request_data),
            mimetype="text/event-stream"
        )
        response.headers.add_header("Cache-control", "no-cache")
        response.headers.add_header("Connection", "keep-alive")
