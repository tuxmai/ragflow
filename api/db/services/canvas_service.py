...
class UserCanvasService(CommonService):
    ...
    def get_by_tenant_ids(
        ...
        fields = [cls.model.id, cls.model.avatar, cls.model.title, cls.model.dsl, cls.model.description, cls.model.permission, User.nickname, User.avatar.alias("tenant_avatar"), cls.model.update_time]
        if keywords:
            agents = (
...
def completionOpenAI(tenant_id, agent_id, question, session_id=None, stream=True, **kwargs):
    ...
    if stream:
            ...
            for ans in canvas.run(stream=True, bypass_begin=True):
                if ans.get("running_status"):
                    ...
                    yield (
                        "data: "
                        + json.dumps(
                            get_data_openai(id=session_id, model=agent_id, content=ans["content"], object="chat.completion.chunk", completion_tokens=completion_tokens, prompt_tokens=prompt_tokens),
                            ensure_ascii=False,
                        )
                        + "\n\n"
                    )
        ...
        try:
            ...
            API4ConversationService.append_message(conv.id, conv.to_dict())
            yield get_data_openai(
                id=session_id, model=agent_id, content="**ERROR**: " + str(e), finish_reason="stop", completion_tokens=len(tiktokenenc.encode("**ERROR**: " + str(e))), prompt_tokens=prompt_tokens
            )
