...
class HTTPClientManager:
    ...
    def _add_authentication_headers(prepared_request: requests.PreparedRequest, original_kwargs: Dict[str, Any]) -> None:
        ...
        if original_kwargs.get("data") and isinstance(original_kwargs["data"], dict):
            form_data = urlencode(
                sorted(original_kwargs["data"].items()), 
                quote_via=quote, 
                safe="-._~"
            ).encode("ascii")
    ...
    def deep_merge_dictionaries(base_dict: Dict[str, Any], override_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        while merge_stack:
            ...
            for key, value in current_override.items():
                if (key in current_base and 
                    isinstance(value, dict) and 
                    isinstance(current_base[key], dict)):
...
class EmbeddingModelValidator:
    ...
    def verify_model_availability(embedding_model_id: str, tenant_id: str) -> Tuple[bool, Optional[Response]]:
        ...
        try:
            ...
            is_tenant_authorized = any(
                model["llm_name"] == model_name and 
                model["llm_factory"] == model_factory and 
            ...
            if not (is_builtin_model or is_tenant_authorized or model_exists_in_service):
                error_response = ResponseBuilder.build_error_response(
                    f"Unsupported embedding model: {embedding_model_id}",
            ...
            if not (is_builtin_model or is_tenant_authorized):
                error_response = ResponseBuilder.build_error_response(
                    f"Unauthorized access to embedding model: {embedding_model_id}",
                    settings.RetCode.ARGUMENT_ERROR
                )
                return False, error_response
            ...
            error_response = ResponseBuilder.build_error_response(
                "Database operation failed during model validation"
            )
            return False, error_response
    ...
    def decorated_function(*args, **kwargs):
        ...
        if not is_valid:
            return ResponseBuilder.build_error_response(
                error_message, 
                settings.RetCode.AUTHENTICATION_ERROR
            )
...
def validate_request(*required_args, **field_constraints):
    ...
    def decorator(function):
        ...
        def decorated_function(*args, **kwargs):
            ...
            is_valid, missing_fields = RequestValidator.validate_required_fields(
                request_data, list(required_args)
            )
            ...
            is_valid, constraint_errors = RequestValidator.validate_field_values(
                request_data, field_constraints
            )
        ...
        def decorated_function(*args, **kwargs):
            ...
            for param in forbidden_params:
                if param in request_data:
                    return ResponseBuilder.build_error_response(
                        f"Parameter '{param}' is not allowed",
                        settings.RetCode.ARGUMENT_ERROR
                    )
...
def handle_server_error(error: Exception) -> Response:
    ...
    if len(error.args) > 1:
        return ResponseBuilder.build_error_response(
            str(error.args[0]), 
            settings.RetCode.EXCEPTION_ERROR
        )
    
    error_message = str(error)
    if "index_not_found_exception" in error_message:
        return ResponseBuilder.build_error_response(
            "No chunks found. Please upload and parse files first.",
            settings.RetCode.EXCEPTION_ERROR
        )
